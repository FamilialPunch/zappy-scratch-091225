...
export default function NewConsultationPage() {
  ...
  useEffect(() => {
    ...
    const role = localStorage.getItem('userRole') || 'provider';
    setUserRole(role);
  }, []);
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-3 rounded-lg border-2 font-medium transition-all ${
                      urgency === level.value
                        ? `border-${level.color}-600 bg-${level.color}-50 text-${level.color}-700`
                        : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
                    {level.label}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                If no provider is selected, the consultation will be automatically assigned based on availability and specialty.
              </p>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div><strong>Type:</strong> {consultationTypes.find(t => t.value === consultationType)?.label}</div>
                <div><strong>Chief Complaint:</strong> {chiefComplaint}</div>
                <div><strong>Symptoms:</strong> {symptoms.join(', ')}</div>
                <div><strong>Duration:</strong> {symptomDuration.replace('_', ' ')}</div>
                <div><strong>Severity:</strong> {severity}/10</div>
                <div><strong>Urgency:</strong> {urgency}</div>
                <div><strong>Provider:</strong> {assignedProvider || 'Auto-assign'}</div>
              </div>
            </div>
          </div>
        );
